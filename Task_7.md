...
Ответ: `10_35`
> Доподлинно верный ответ неизвестен - возможно верным является `12_35`
...
### 3
> То, что LLM показывает себя достойно при сканировании на уязвимости, не гарантирует её полную безопасность. Нужно использовать дополнительную защиту — инструмент класса guardrails.
>
> Протестируй инструмент llm guard на возможность анонимизации персональных данных, которые ML-модель выдаёт в ответ на пользовательские запросы. Коллега помог тебе написать скрипт на Python llmguarder.py. Запусти его и определи, на какую маску llm guard заменяет персональные данные из тестового примера.
>
> Команда для запуска: `python llmguarder.py`

> Маска ответа: (вводи полностью, включая кавычки)
<*****_*******>

Рассматриваемый в рамках задания скрипт не поднимает модель сам, поэтому прежде её необходимо запустить:
```
ollama run gemma:latest
```

Далее можно запустить непосредственно сам скрипт
```
python llmguarder.py
```

В ходе выполнения скрипта будет сформирован запрос к модели, содержащий чувствительную информацию. Ответ модели будет представлять собой краткий рассказ, в котором чувствительная информация будет сохранена.  
Инструмент LLM Guard перехватывает вывод модели и форматирует его, заменяя чувствительную информацию на маску.

<p align="left">
  <img src="https://github.com/AronHopeless/CyberCamp2025_guide/blob/main/imgs/7-12.jpg">
</p>

Используемая маска представлена на скриншоте выше
